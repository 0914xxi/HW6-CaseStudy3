## R Shiny Dashboard of “Case Study3” ##
...
ui <- dashboardPage(
  ...
  #Dashboard title
  dashboardHeader(title = 'Project by Xi Xi', titleWidth = 290),
  
  #Sidebar layout
  dashboardSidebar(width = 290,
                   sidebarMenu(menuItem("About", tabName = "about", icon = icon('list-alt')),
                               menuItem("Tutorial", tabName = "tutorial", icon = icon('location-arrow')),
  ...
  dashboardBody(tags$head(tags$style(HTML('.main-header .logo {font-weight: bold;}'))),
                tabItems(
                  ...
                  #About tab content
                  tabItem('about',
                          box(title = 'Bike Sharing (HW6-Case Study3) ', status = 'success', width = 12, height = 222,
                              helpText('This project is about bike sharing.'),
                              helpText('Sharing economy is becoming more and more popular now. Bike share programs provide a great number of social, economic, and health benefits, such as Flexible transport, reduction of vehicle emissions, lowered road congestion, affordable daily commutes, and ease of accessibility.'),
                              helpText(sprintf('All raw data is from the bike share system called “Capital Bikeshare” between 2011 and 2012. We aim to analyze the data to help the company predict the number of bike registrations for a specific day and weather conditions.',
                                               round(mae_rf, digits = 0), round(rmse_rf, digits = 0))))),
                  
                  #Tutorial tab content
                  tabItem('tutorial',
                          box(title = 'How to use?', status = 'success', width = 12, height = 338,
                              helpText('There are three function parts in the application.'),
                              helpText('➢The first part is about two plots of the total values, which are under different conditions from 2011 to 2012. You can filter the numerical variables (temperature, feeling temperature, humidity, wind speed, casual registrations, new registrations, and total registrations) to build the histogram plot. And you also can filter the counts of registrations by time, type of registration, and weather conditions to build the bar frequency plot.'),
                              helpText('➢The second part is about two graphs of the dashboard. You can filter the graph by year and type of registration to create a line plot visualizing the number of registrations per month. Additionally, you can filter the graph by year, type of registration, and weather choice to create a bar plot.
                            '),
                              helpText(sprintf('➢The third part is about predicting the number of bike registrations in 2020. Because it is the dependent variable, that means we are in the presence of a regression task. After calculation, the multiple linear regression model is used to predict. Compared with other algorithms, it has more accurate results. You can input the specific date and weather values, and then click the “Calculate” button to get the results. The outcome shows a total number of registrations for the specific date and weather conditions. In addition, the outcome reveals the range of variation in the predicted values. ', round(mae_rf, digits = 0), round(rmse_rf, digits = 0))))),
                  ...
                  tabItem('plots', 
                         ...
                         tabItem('pred',
                                 ...
                                 box(title = 'Model explanation', status = 'success', width = 8, height = 260,
                                     helpText('The prediction is based on a multiple linear regression supervised machine learning model.'),
                                     helpText('It is a statistical technique that uses several explanatory variables to predict the outcome of a response variable.'),
                                     helpText('The name of the dataset used to train the model is "Bike Sharing Dataset", taken from the UCI Machine Learning Repository website.'),
                                     helpText(sprintf('The result will be forecasted by the total number of bikes rented on a specific day of the week, hour, and weather conditions.', round(mae_rf, digits = 0), round(rmse_rf, digits = 0)))))
                )
  )
)
...
server <- shinyServer(function(input, output) {
  ...
  output$histPlot <- renderPlot({
    ...
    ggplot(data = bike, aes(x = bike[[num_val]]))+ 
      geom_histogram(stat = "bin", fill = 'steelblue', color = 'lightgrey')+
  ...
  output$freqPlot <- renderPlot({
    ...
    ggplot(data = bike, aes(x = bike[[cat_val]]))+
      geom_bar(stat = 'count', fill = 'palevioletred', 
               width = 0.5)+
  ...
  output$barPlot <- renderPlot({
    ...
    ggplot(weather, aes(x = season, y = weather[[regis_val]], 
      ...
      scale_fill_manual(values = c('Bad' = 'salmon2', 'Fair' = 'skyblue', 'Good' = 'mediumseagreen', 'Very Bad' = 'tomato4'),
                        name = 'Weather')
